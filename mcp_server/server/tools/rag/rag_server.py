import os
import requests
from mcp_server.config.constants import (CHROMA_PATH, EMBED_MODEL, OLLAMA_BASE_URL,
...
def process_single_pdf(pdf_path: str) -> int:
    ...
    # Even we dun provide this ids the vector store still create internal id
    # so it is optional 
...
def download_pdf(url: str, download_dir: str = "./downloads") -> str:
    ...
    # Download the file
    response = requests.get(url, stream=True) # stream for large file
    response.raise_for_status() # http status code 200 ok
    
    with open(local_path, 'wb') as f: # write binary
        for chunk in response.iter_content(chunk_size=8192):
            f.write(chunk)
    
    return local_path # return that stored download path
    # we need this file path to parse as input of process_single_pdf
...
async def retrieve(query: str, n: int = 5) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        for doc, score in results:
            ...
            }) # useful for llms to know matched percentage
...
async def db_info() -> Dict[str, Any]:
    ...
    try:
        ...
        sources = set() # count that less than 100, take it into the sample 
        # use set() because want only unique of source will be added
        ...
        } # return back to LLMs so it know overall database information, so it can inform user back
...
async def clear_db() -> Dict[str, Any]:
    ...
    try:
        ...
        # both delete and recreate are global vector store
        return {
            "status": "success",
